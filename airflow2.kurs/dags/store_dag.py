...
with DAG('store_dag',default_args=default_args,schedule_interval='@daily', template_searchpath=['/usr/local/airflow/sql_files'], catchup=False) as dag:
    ...
    """t1=FileSensor(
        ...
        # ui connectiondan dan filesensor conn_id sine bakabilirsin varsayılan olarak mevcut bu connection
        poke_interval=5,
        #konumdaki data mevcut değilse 5 saniye aralıklkarla o konuma bakmaya devam eder taki timeoutu aşana kadar
        timeout=25,
        # timeout a kadar dosya bulunamdıysa task fail olur
        soft_fail=True 
        # konumda dosya bulunamadıysa  bu task fail olur sonraki tasklerin ın durumu skip olur
        )"""
...
#csv dosyamı kontrol etmek için task
# shasum komutu geriye bir shasum değeri döndürür eğer belirttiğim komutda dosya varsa değilse hata döner
#datayı temizlemek için task 2 
