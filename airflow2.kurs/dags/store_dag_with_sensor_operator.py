...
with DAG('store_dag_with_sensor',default_args=default_args,schedule_interval='@daily', template_searchpath=['/usr/local/airflow/sql_files'], catchup=False) as dag:
    ...
    t1 = FileSensor(
        ...
        timeout=25,
        soft_fail=True
        #konumda dosya bulunamadıysa  bu task ve  sonraki tasklerin ın durumu skip olur
    )
